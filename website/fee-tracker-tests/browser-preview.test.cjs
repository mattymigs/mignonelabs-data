...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('[data-stat="total"]').textContent === '20');
    check('private snapshot renders 20 municipalities', await page.locator('tbody tr').count() === 20);
    ...
    check('corrected public status labels', await page.locator('[data-stat="full"] + span').innerText() === 'Reported full/substantial relief');
    ...
    await page.locator('[name="municipality"]').fill('Berkeley');
